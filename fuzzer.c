...
void init_forkserver(char** argv) {
  ...
  printf("Spinning up the fork servers..\n");
  ...
  if (!forksrv_pid) {
    ...
    // point stdout to dev_null_fd
    dup2(dev_null_fd, 1);
    // point stdin to dev_null_fd
    dup2(dev_null_fd, 2);
    ...
    // fprintf(stderr, "forky exec");
    execv(target_path, argv);
    // fprintf(stderr, "donezo forking");
    /* Use a distinctive bitmap signature to t64ell the parent about execv()
       falling through. */
...
static u8 run_target(int timeout) {
  ...
  MEM_BARRIER();
    ...
    // fprintf(stderr, "After Memset2 %d\n", child_pid);
...
static void check_cpu_governor(void) {
  ...
  printf("cd /sys/devices/system/cpu\n");
  printf("echo performance | tee cpu*/cpufreq/scaling_governor\n\n");
...
void gen_mutate(){
    ...
    for(int iter=0 ;iter<13; iter=iter+1){
        ...
        for(int step=0;step<low_step;step=step+1){
            ...
            int ret = has_new_bits(virgin_bits);
            if(ret == 2){
                char* mut_fn = alloc_printf("%s/id_%d_%d_%06d_cov_low", out_dir, round_cnt, iter, mut_cnt);
                int mut_fd = open(mut_fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
                ck_write(mut_fd, out_buf2, len, mut_fn);
                close(mut_fd);
                // fprintf(stderr, "im here7");
                free(mut_fn);
                mut_cnt = mut_cnt + 1;
            }
            if(ret == 1){
                char* mut_fn = alloc_printf("%s/id_%d_%d_%06d_new_low", out_dir, round_cnt, iter, mut_cnt);
                int mut_fd = open(mut_fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
                ck_write(mut_fd, out_buf2, len, mut_fn);
                close(mut_fd);
                // fprintf(stderr, "im here8");
                free(mut_fn);
                mut_cnt = mut_cnt + 1;
            }
    ...
    for(int del_count=0; del_count < 1024;del_count= del_count+1){
        ...
        int ret = has_new_bits(virgin_bits);
        if(ret==2){
            char* mut_fn = alloc_printf("%s/id_%d_%06d_cov_rand", out_dir,round_cnt, mut_cnt);
            int mut_fd = open(mut_fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
            ck_write(mut_fd, out_buf1, len-cut_len, mut_fn);
            // printf(stderr, "im here11");
            free(mut_fn);
            close(mut_fd);
            mut_cnt = mut_cnt + 1;
        }
        else if(ret==1){
            char* mut_fn = alloc_printf("%s/id_%d_%06d_new_rand", out_dir,round_cnt, mut_cnt);
            int mut_fd = open(mut_fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
            ck_write(mut_fd, out_buf1, len-cut_len, mut_fn);
            // printf(stderr, "im here12");
            free(mut_fn);
            close(mut_fd);
            mut_cnt = mut_cnt + 1;
        }
        ...
        // fprintf(stderr, "%d", fault);
        if (fault != 0){
            if(fault == FAULT_CRASH){
                char* mut_fn = alloc_printf("%s/crash_%d_%06d", "./crashes",round_cnt, mut_cnt);
                int mut_fd = open(mut_fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
                ck_write(mut_fd, out_buf3, len+cut_len, mut_fn);
                // fprintf(stderr, "im here13");
                free(mut_fn);
                close(mut_fd);
                mut_cnt = mut_cnt + 1;
            }
        ...
        if(ret == 2){
            char* mut_fn = alloc_printf("%s/id_%d_%06d_cov_randc", "vari_seeds",round_cnt, mut_cnt);
            int mut_fd = open(mut_fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
            ck_write(mut_fd, out_buf3, len+cut_len, mut_fn);
            // fprintf(stderr, "im here15");
            free(mut_fn);
            close(mut_fd);
            mut_cnt = mut_cnt + 1;
        }
        else if(ret == 1){
            char* mut_fn = alloc_printf("%s/id_%d_%06d_new_randc", "vari_seeds",round_cnt, mut_cnt);
            int mut_fd = open(mut_fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
           
           if(mut_fd == -1){
              // fprintf(stderr, "Oh dear, something went wrong with read()! %s\n", strerror(errno));
              mut_fn = alloc_printf("%s/id_%d_%06d_new_randc_2", "vari_seeds",round_cnt, mut_cnt);
              mut_fd = open(mut_fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
           }

           
            // fprintf(stderr, "File: %d\n", mut_fd);
            // fprintf(stderr,"%s/id_%d_%06d_new_randc\n", "vari_seeds",round_cnt, mut_cnt);
            ck_write(mut_fd, out_buf3, len+cut_len, mut_fn);
            // fprintf(stderr, "im here16\n");
            free(mut_fn);
            // fprintf(stderr, "post-im here16\n");
            close(mut_fd);
            mut_cnt = mut_cnt + 1;
        }
    }
...
void gen_mutate_slow(){
    ...
    for(int del_count=0; del_count < 4096;del_count= del_count+1){
        ...
        if(ret==2){
            ...
            char* mut_fn = alloc_printf("%s/id_%d_%06d_new", out_dir,round_cnt, mut_cnt);
            int mut_fd = open(mut_fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
            ck_write(mut_fd, out_buf1, len-cut_len, mut_fn);
            free(mut_fn);
            close(mut_fd);
...
void dry_run(char* dir, int stage){
        ...
        if(S_ISREG(statbuf.st_mode)) {
          // fprintf(stderr, entry->d_name);
            ...
            if(tmp != entry->d_name){
                ...
                if (ret!=0){
                    if(stage == 1){
                        char* mut_fn = alloc_printf("../%s/id_%d_%06d_dry", out_dir,round_cnt, mut_cnt);
                        int mut_fd = open(mut_fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
                        ck_write(mut_fd, out_buf1, len, mut_fn);
                        free(mut_fn);
                        close(mut_fd);
    ...
    if(stage ==2 ){
        ...
        fprintf(stderr, "avg %d time out %d cnt %d sum %lld \n.",(int)avg_us, exec_tmout, cnt,total_cal_us);
    }

    fprintf(stderr, "dry run %ld edge coverage %d.\n", total_execs,count_non_255_bytes(virgin_bits));
    return;
}
...
void copy_seeds(char * in_dir, char * out_dir, int f_len){
    struct dirent *de;
    DIR *dp;
    FILE *fptr1;
    // fptr1 = fopen(src, "r");
    ...
    while((de = readdir(dp)) != NULL){ 
        ...
        long int fileSize = ftell(fptr1);
        fclose(fptr1);

        // Skip files that are too large to runs
        if (fileSize > f_len){;
...
void fuzz_lop(char * grad_file, int sock){
    ...
    char *line = malloc(chunk_len);

    if(line == NULL){
      perror("Unable to allocate memory for line buffer");
      exit(1);
    }
    // fprintf(stderr, "Grad File Read\n");
    // char *line = NULL;
    size_t llen = 0;
    ssize_t nread = NULL;
    int line_cnt = 0;
    ...
    line[0] = '\0';
     ...
     while(fgets(chunk, sizeof(chunk), stream) != NULL){
      ...
      if(chunk_len - len_used < chunk_used){
        chunk_len *= 2;
        if((line = realloc(line, chunk_len)) == NULL){
      ...
      strncpy(line + len_used, chunk, chunk_len - len_used);
      len_used += chunk_used;

      // when code reaches new line
      if(line[len_used - 1] == '\n'){
        
        line_cnt = line_cnt+1;
        // fprintf(stderr, "New Gradient Line\n\n");
        if(line_cnt == retrain_interval){
            ...
            if((edge_gain > 30) || (fast == 0)){
                send(sock,"train", 5,0);
                fast = 1;
                fprintf(stderr, "fast stage\n");
            }
            else{
                send(sock,"sloww",5,0);
                fast = 0;
                fprintf(stderr, "slow stage\n");
            }
        }
        // fprintf(stderr, "Parsing Gradient\n");
        /* parse gradient info */
        ...
        fprintf(stderr, "File Testing: %s \n", fn);
        // fprintf(stderr, "Sign Testing: %s \n", loc_str);
        // fprintf(stderr, "Loc Testing: %s \n", sign_str);


        parse_array(loc_str,loc);
        parse_array(sign_str,sign);
        // fprintf(stderr,"Donezi\n");
        /* print edge coverage per 10 files*/
        if((line_cnt % 10) == 0){ 
            fprintf(stderr, "$$$$&&&& fuzz %s line_cnt %d\n",fn, line_cnt);
            fprintf(stderr, "edge num %d\n",count_non_255_bytes(virgin_bits));
            fflush(stdout);
        }
...
void start_fuzz_test(int f_len){
    ...
    /* set up buffer */
    out_buf = malloc(50000);
    if(!out_buf)
        perror("malloc failed");
    out_buf1 = malloc(50000);
    if(!out_buf1)
        perror("malloc failed");
    out_buf2 = malloc(50000);
    if(!out_buf2)
        perror("malloc failed");
    out_buf3 = malloc(50000);
    if(!out_buf3)
        perror("malloc failed");
...
void main(int argc, char*argv[]){
    ...
    switch (opt) {
      ...
      case 'l': /* file len */
         ...
         fprintf(stderr, "num_index %d %d small %d medium %d large %d\n", num_index[12], num_index[13], havoc_blk_small, havoc_blk_medium, havoc_blk_large);
         fprintf(stderr, "mutation len: %ld\n", len);
         break;
