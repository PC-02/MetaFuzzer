...
def Train_LSTM(model):
    ...
    # train model for batch size of 16 using generator
    model.fit(x, y, epochs=3, verbose=1, batch_size=samples, callbacks=callbacks_list)
...
def Create_Model():
    ...
    ## play around with all hyperparameters
...
def Gen_Gradient_Train(index, randSeeds, m, layerList, idx, sign):
    ...
    global MAX_FILE_SIZE
    # Loss Values
    # print(layerList[-2][1].output)
    # print(layerList[-2][1].output.shape)
...
def Gen_Mutate_Info(model, edgeNum, train, modelType):
    ...
    if seedLen > edgeNum:
        randSeed1 = [newSeedList[i] for i in np.random.choice(seedLen, edgeNum, replace=False)]
        randSeed2 = [newSeedList[i] for i in np.random.choice(seedLen, edgeNum, replace=False)]
    else:
        randSeed1 = [newSeedList[i] for i in np.random.choice(seedLen, edgeNum)]
        randSeed2 = [newSeedList[i] for i in np.random.choice(seedLen, edgeNum)]
    ...
    with open('gradient_info_p', 'w') as f:
        for idx in range(edgeNum):
            ...
            if modelType == "LSTM":
                writeList = Gen_Gradient_Train_LSTM(index, randSeeds, model, layerList, idx, train)
            else:
                 writeList = Gen_Gradient_Train(index, randSeeds, model, layerList, idx, train)
...
def Start_Server():
    ...
    start = time.time()
    Gen_Gradient(first=True, trained=trained, modelType=model_type, bitmaps=created)
    ...
    while True:
        ...
        if not data:
            ...
            Gen_Gradient(first=False, trained=trained,  modelType=model_type, bitmaps=created)
            conn.sendall(b"start")
    conn.close()
